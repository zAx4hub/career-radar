"""career-radar engine — zAx4hub (algo=markov)."""
...
def rank(text: str, seed: int = 76) -> float:
    toks = _tokens(text)
    if not toks:
        return 0.0
...
def run(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = payload or {}
    items = payload.get("items") or [{"text": "career-radar"}]
    threshold = float(payload.get("threshold", 0.35))
    seed = int(payload.get("seed", 76))
...
def demo() -> dict[str, Any]:
    return run(
        {
            "items": [
                {"text": "Job search CRM + skill gaps"},
                {"text": "zAx4hub quality gate regression fixture"},
                {"text": "deterministic markov scoring path"},
            ],
            "threshold": 0.2,
...
def inspect() -> dict[str, Any]:
    return {
        "name": "career-radar",
        "author": "zAx4hub",
        "oneLiner": "Job search CRM + skill gaps",
        "algo": "markov",
        "version": "0.1.0",
    }
